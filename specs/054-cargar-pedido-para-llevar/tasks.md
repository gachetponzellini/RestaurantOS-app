...
- [x] **T018** Selector de **cliente existente** en la vista datos: input con debounce (300ms) → `buscarClientes` → lista; al elegir prellena nombre + teléfono (editable). Cargar cliente nuevo a mano sigue disponible.
- [x] **T020** **Dirección guardada del cliente** (pedido de Juan): al elegir un cliente en delivery, `getClienteDirecciones(slug, customerId)` (nuevo action, scope tenant vía el customer + gate `canCargarPedido`) trae sus `customer_addresses` y **prellena** el campo — **editable** (chips por dirección si tiene varias + input libre siempre editable). + 3 tests. Verde: typecheck + `customers-actions.test` 7✓ + build.
