...
CREATE OR REPLACE FUNCTION "public"."march_due_scheduled_orders"() RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
declare
...
ALTER FUNCTION "public"."march_due_scheduled_orders"() OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."send_due_shift_summaries"() RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
declare
...
ALTER FUNCTION "public"."send_due_shift_summaries"() OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."comandas" (
    ...
    "print_failed_at" timestamp with time zone,
...
COMMENT ON COLUMN "public"."comandas"."print_failed_at" IS 'Spec 33: marca que el print agent no pudo imprimir esta comanda. Se setea al primer fallo reportado (dedup del aviso comanda.impresion_fallida) y se limpia al confirmar la impresión. Null = sin fallo pendiente.';
...
CREATE TABLE IF NOT EXISTS "public"."daily_menu_components" (
    ...
    "extra_price_cents" bigint DEFAULT 0 NOT NULL,
    CONSTRAINT "daily_menu_components_extra_price_cents_check" CHECK (("extra_price_cents" >= 0)),
...
COMMENT ON COLUMN "public"."daily_menu_components"."extra_price_cents" IS 'Adicional en centavos de esta opción (kind=choice). Se suma al order_item padre del combo; los hijos quedan en $0. Default 0 = incluida. Spec 29.';
...
COMMENT ON COLUMN "public"."invoices"."cancelled_by" IS 'Spec 34: usuario que anuló el comprobante. Pareja con cancelled_reason. Null en anulaciones previas a la migración.';
...
COMMENT ON COLUMN "public"."order_items"."cancelled_by" IS 'Spec 34: usuario que anuló el ítem. Pareja con cancelled_reason. Null en anulaciones previas a la migración.';
...
COMMENT ON COLUMN "public"."orders"."scheduled_at" IS 'Pedido diferido (spec 31): fecha/hora futura de retiro. Null = para ahora. Si es futuro, el pedido no marcha hasta ~40 min antes (cron) o "marchar ahora".';



COMMENT ON COLUMN "public"."orders"."cancelled_by" IS 'Spec 34: usuario que anuló la orden/mesa. Pareja con cancelled_reason. Null en anulaciones previas a la migración o sin actor.';
...
ALTER TABLE "public"."shift_summary_sends" OWNER TO "postgres";


COMMENT ON TABLE "public"."shift_summary_sends" IS 'Spec 34: marca anti-doble-envío del resumen de cierre. Una fila por (negocio, día AR) cuando el mail salió OK. El cron saltea si ya existe; "enviar ahora" reenvía sin consultar.';
...
ALTER TABLE ONLY "public"."shift_summary_sends"
    ADD CONSTRAINT "shift_summary_sends_pkey" PRIMARY KEY ("business_id", "sent_for_date");
...
CREATE INDEX "orders_business_scheduled_at_idx" ON "public"."orders" USING "btree" ("business_id", "scheduled_at") WHERE ("scheduled_at" IS NOT NULL);
...
ALTER TABLE ONLY "public"."order_items"
    ADD CONSTRAINT "order_items_cancelled_by_fkey" FOREIGN KEY ("cancelled_by") REFERENCES "auth"."users"("id") ON DELETE SET NULL;
...
ALTER TABLE ONLY "public"."shift_summary_sends"
    ADD CONSTRAINT "shift_summary_sends_business_id_fkey" FOREIGN KEY ("business_id") REFERENCES "public"."businesses"("id") ON DELETE CASCADE;
...
GRANT ALL ON FUNCTION "public"."march_due_scheduled_orders"() TO "anon";
GRANT ALL ON FUNCTION "public"."march_due_scheduled_orders"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."march_due_scheduled_orders"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."send_due_shift_summaries"() TO "anon";
GRANT ALL ON FUNCTION "public"."send_due_shift_summaries"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."send_due_shift_summaries"() TO "service_role";
...
GRANT ALL ON TABLE "public"."shift_summary_sends" TO "anon";
GRANT ALL ON TABLE "public"."shift_summary_sends" TO "authenticated";
GRANT ALL ON TABLE "public"."shift_summary_sends" TO "service_role";
...
-- ─────────────────────────────────────────────────────────────────────────
-- Cron schedules (pg_cron). El `db dump` no captura `cron.job`; se re-agregan
-- acá para que el baseline reproduzca el estado completo. Las funciones
-- (march_due_scheduled_orders / send_due_shift_summaries) ya están definidas
-- arriba en este dump; pg_cron ya está como extensión. Dormidos sin las GUCs
-- app.settings.cron_base_url / cron_secret.
-- ─────────────────────────────────────────────────────────────────────────
select cron.schedule('orders-march-scheduled', '*/5 * * * *', $$ select public.march_due_scheduled_orders(); $$);
select cron.schedule('shift-summary-send',     '*/15 * * * *', $$ select public.send_due_shift_summaries(); $$);
